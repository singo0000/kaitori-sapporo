...
- [x] PCで見たときのピンク文字が残っている箇所の最終確認
  ...
  - **【解決済】Tailwind v4 の `@theme inline` の影響で一部ブラウザでリンクが既定の「訪問済み色（紫/ピンク）」になっていたバグを根本解決。（Tailwind構文を修正し、サイト全体の色をブルー基調に再統一）。**
- [x] ブログトップのメディア化（パンくずリスト・ヘッダー装飾）
- [x] スマホ向けスライドナビ・自動目次（TOC）の実装
  - スマホでの使い勝手を考慮した横スクロールの『合わせて読みたい』・カテゴリーナビゲーションを追加。
  - 記事中の見出し(h2, h3)を自動抽出する目次機能を実装。
- [x] CTAの電話番号(050-1724-2478)・24時間年中無休表記の反映
